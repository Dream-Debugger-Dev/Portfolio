...
function spawnShootingStar() {
    const count = Math.random() < 0.3 ? 2 : 1; // 30% chance of double
    for (let i = 0; i < count; i++) {
        shootingStars.push(new ShootingStar());
    }
    const nextDelay = (Math.random() * 5000 + 5000); // 5-10 sec
    setTimeout(spawnShootingStar, nextDelay);
}
// First one after 2-4 seconds
...
function animateCosmos() {
    ...
    // Nebula clouds (subtle breathing glow)
